...
CREATE TABLE unit (
    ...
    floor_number INT,
    unit_type VARCHAR(20) CHECK (
        unit_type IN ('studio', '1b', '2b', '3b', 'house', 'room')
    ),
...
CREATE TABLE billing(
    ...
    amount DECIMAL(10, 2) NOT NULL,
    paid_for VARCHAR(20) DEFAULT 'rent' CHECK (
        paid_for IN (
            'water',
            'electricity',
            'garbage',
...
CREATE TABLE unitInspection (
    ...
    inspection_type VARCHAR(20) CHECK (
        inspection_type IN ('monthly', 'move_in', 'move_out')
    ),
...
CREATE TABLE damage (
    ...
    status VARCHAR(20) DEFAULT 'reported' CHECK (
        status IN ('reported', 'assessed', 'in_repair', 'completed')
    ),
    repaired_date DATE,
...
BEGIN -- Create the lease and get the lease_id
INSERT INTO leases (
        unit_id,
        tenant_id,
        start_date,
